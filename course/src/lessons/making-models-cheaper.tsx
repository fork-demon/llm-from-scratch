  ...
  return (
    <Lesson id="making-models-cheaper">
      <Why>
        ...
        <p>One fact from <a href="#/lesson/inference-systems">Serving many users at once</a> carries this lesson: <b>decode is memory-bound</b>. To produce one token the GPU reads every weight once, and the arithmetic takes roughly 1% of the time the read takes. For one stream, tokens per second is at most <span className="mono">bandwidth ÷ weight bytes</span>. (Prefill is the opposite: a whole prompt in one pass, and the arithmetic units are the limit.)</p>
        <p>Why not buy more bandwidth? It did grow, from about 2 TB/s on an A100 to about 8 TB/s on a B200, four times in two generations. But 16-bit arithmetic grew about seven times over the same span, and models grew faster than either. No bandwidth helps a model that does not fit in the memory you have.</p>
        <p>So three things are left on the table:</p>
        <ul>
          <li><b>Store each weight in fewer bytes.</b> Read a quarter of the bytes and the step is up to four times faster, and the freed memory holds more KV cache. The price is rounding error.</li>
          <li><b>Get more than one token per pass.</b> The arithmetic units are nearly idle during decode, so let something cheap guess the next few tokens and have the big model check them all in one pass.</li>
          <li><b>Use more than one GPU.</b> When 140 GB of weights meet an 80 GB card, no amount of scheduling helps. The model has to be cut up.</li>
        </ul>
        <p>Fewer bytes makes the step shorter. Spare arithmetic makes the step do more. Both attack the same number from opposite ends.</p>
      </Why>
      ...
      <MentalModel title="Fewer bytes, more tokens per pass, more GPUs">
        ...
        />
        <p><b>int8</b> and <b>int4</b> have 256 and 16 possible values per weight; a 16-bit float has tens of thousands. Because decode is limited by bytes moved, this makes serving <b>smaller and faster at once</b>. It is lossy compression of a table you read in full on every request, and decompression is almost free because the arithmetic units were idle anyway.</p>
        ...
        <p>The fix is more scales, each covering fewer weights. This is the <b>granularity</b>: one scale per tensor, then one per row (per output channel), then one per group of 32 to 128 weights. Each scale costs 16 bits of its own, so group-wise int4 really costs about 4.1 to 4.5 bits per weight.</p>
        <DeepDive title="The integer methods named on model cards">
        ...
        <p>In data centres today the main route is <b>tiny floating-point formats</b>, because recent GPUs do their arithmetic on them directly. A float spends some bits on an <b>exponent</b> (how big) and the rest on a <b>mantissa</b> (how precise); with 8 or 4 bits, you choose the split.</p>
        <Term
        ...
        <DeepDive title="Microscaling FP4, worked on four weights">
          <Term
        ...
        <p>The big model can score 5 positions in one pass in nearly the time it scores 1, exactly like a tiny prefill. It only needs to know which 5 tokens to score.</p>
        <p>Amma, when Riya explains this on the phone: “Like a student who writes out the whole sum and brings it to me. I tick five lines in the time it takes to check one.” At the first wrong line she corrects it and crosses out everything below. Where the analogy stops: Amma’s ticking is yes or no, while the real check is a probability rule.</p>
        <Term
          ...
          formal={<>Accept a proposed token x with probability min(1, p(x) ÷ q(x)), where p is the target’s probability and q the draft’s; on the first rejection, draw a replacement from max(0, p − q) rescaled, and discard the later guesses (details in The math). Leviathan et al. (ICML 2023) and Chen et al. (2023) prove the output distribution equals the target’s.</>}
        />
        <p>The <b>target model</b> is the one whose answers you want. The <b>drafter</b> makes the guesses, and there are three common kinds. The acceptance rule is the same for all three.</p>
        <ul>
          <li><b>A separate small model</b>, a few hundred million parameters against the target’s tens of billions: the original recipe, and our demo’s.</li>
        ...
        <div className="table-scroll">
          <table className="plain">
            <thead><tr><th>split</th><th>how</th><th>one request’s latency</th><th>price</th></tr></thead>
            <tbody>
              <tr><td><b>Tensor parallel</b></td><td>every weight matrix, across GPUs</td><td>falls: each GPU reads only its slice</td><td>every layer waits for the GPUs to talk, twice; needs the fast links inside one server</td></tr>
              <tr><td><b>Pipeline parallel</b></td><td>by layers: GPU 1 runs layers 1 to 20, GPU 2 runs 21 to 40, …</td><td>unchanged: one token still visits every stage in turn</td><td>idle gaps called bubbles (GPipe, Huang et al., 2019), filled by keeping several smaller pieces of the batch moving at once; slower links are fine</td></tr>
              <tr><td><b>Data parallel</b></td><td>complete replicas behind a load balancer</td><td>unchanged</td><td>none for latency; it multiplies throughput</td></tr>
            </tbody>
          </table>
        </div>
        <p>A large deployment typically combines them: tensor parallel inside a server so the model fits and responds quickly, then as many replicas as the traffic needs. So the three techniques divide the work: quantization shrinks the bytes, speculative decoding buys more tokens per read of those bytes, and parallelism spreads the bytes over more memory links. A real engine does all of it at once, on top of the scheduler from <a href="#/lesson/inference-systems">the last lesson</a>.</p>
      </MentalModel>
      ...
      <Numbers title="What int4 buys, on the same GPU">
        ...
        <ul>
          <li><b>For one user, almost four times the speed.</b> The weight read is the whole step, and it shrank by the full factor. This is the gain you notice on a laptop.</li>
        ...
        <p>That gap is the thing to remember, and it is why <b>quantizing the KV cache</b> is a separate and increasingly important lever at scale. Also, the right column is a more optimistic ceiling than the left: real int4 kernels spend arithmetic unpacking weights. Treat 480 as “the read is no longer what limits you”, not as a number to promise anyone.</p>
      </Numbers>
      ...
      <TheMath>
        ...
        <DeepDive title="Why the rule is exact, even with a bad draft">
      ...
      <CodeIt>
        ...
        <Code
          ...
          title="symmetric absmax quantization"
          setup={`import numpy as np`}
          show={`w = np.array([0.021, -0.035, 0.010, 0.004, -0.012, 0.030, -0.007, 0.018])   # max|w| = 0.035
for bits in (8, 4):
    q, scale = quantize_block(w, bits)
    print(f"int{bits}: scale {scale:.5f}  stored {q.tolist()}")
    print("       read back", (q * scale).round(4).tolist())

q, scale = quantize_block(np.array([0.06, -0.02, 0.012, 1.4]), 4)   # one outlier
print("int4 with an outlier:", q.tolist())`}
        >{`
def qmax(bits):
...
`}</Code>
        <Code
          source="phase6-engineering/quantize_demo.py"
          title="group-wise: one scale per `group` consecutive weights in a row"
          setup={`import numpy as np
def qmax(bits):
    return 2 ** (bits - 1) - 1

rng = np.random.default_rng(0)
rows, cols, group, bits = 4, 64, 16, 4
W = rng.normal(0, 0.02, size=(rows, cols))   # ordinary weights, typical size 0.02
W[0, 5] = 1.0                                 # one planted outlier`}
          show={`print("scales:", scale.shape, "(one per group of", group, "in each row)")
err = np.abs(W_hat - W)
print("mean error in the outlier's group: ", err[0, :16].mean().round(4))
print("mean error in every other group:   ", np.delete(err.reshape(-1, group), 0, axis=0).mean().round(4))`}
        >{`
...
`}</Code>
      ...
      <Remember
        items={[
          <><b>Quantization stores each weight in fewer bits plus a shared scale</b>: a small integer (int8, int4) or a tiny float (FP8, and FP4 formats such as MXFP4 with one scale per block of 32). Fewer bytes shrink memory and speed up decode. <b>Outliers are what breaks it</b>: more scales, per row or per group of 32 to 128, contain the damage, at about 4.1 to 4.5 bits per weight.</>,
